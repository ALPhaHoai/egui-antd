use egui::{Id, Response, Ui, WidgetText, Color32, Vec2, Align, Layout, Sense, LayerId, Order};
    ...
    pub fn new(key: impl Into<String>, label: impl Into<WidgetText>) -> Self {
        ...
        Self {
            ...
            label: Box::new(move |ui: &mut Ui| {
                ...
                ui.add(egui::Label::new(text).selectable(false).sense(egui::Sense::empty()))
            }),
            closable: true,
            disabled: false,
        }
        ...
        Self {
            ...
            label: Box::new(move |ui: &mut Ui| {
                ui.horizontal(|ui| {
                    ...
                    resp |= ui.add(egui::Label::new(text).selectable(false).sense(egui::Sense::empty()));
                    resp
                }).inner
            }),
            closable: true,
...
impl<'a> Tabs<'a> {
    ...
    pub fn show(mut self, ui: &mut Ui, mut add_contents: impl FnMut(&mut Ui, &str)) -> TabsResponse {
        ...
        let mut active_key = if let Some(key) = &self.active_key {
            ...
            last_active_key.clone()
                .or(self.default_active_key.clone())
        };
        ...
        if active_key.is_some() && last_active_key.is_none() {
            if let Some(key) = &active_key {
                ui.ctx().data_mut(|d| d.insert_temp(id.with("active_key"), key.clone()));
            }
        }
        ...
        let mut res = ui.push_id(id, |ui| {
            ...
            if has_focus {
                let enabled_indices: Vec<usize> = self.panes.iter().enumerate()
                ...
                if !enabled_indices.is_empty() {
                    let current_enabled_pos = active_key.as_ref().and_then(|key| {
                        enabled_indices.iter().position(|&ei| &self.panes[ei].key == key)
                    });
                    ...
                    ui.input(|i| {
                        if is_horizontal {
                            if i.key_pressed(egui::Key::ArrowLeft) {
                                target_enabled_pos = current_enabled_pos.map(|p| p.saturating_sub(1));
                            } else if i.key_pressed(egui::Key::ArrowRight) {
                                target_enabled_pos = current_enabled_pos.map(|p| (p + 1).min(enabled_indices.len() - 1));
                            }
                        } else {
            ...
            if is_horizontal {
                ui.vertical(|ui| {
                    if self.tab_position == TabPosition::Top {
                        edit_action = self.render_horizontal_tab_bar(ui, id, &mut active_key, &mut changed, &mut scroll_to_active);
                        ui.add_space(12.0);
                ...
                ui.horizontal(|ui| {
                    if self.tab_position == TabPosition::Start {
                        edit_action = self.render_vertical_tab_bar(ui, id, &mut active_key, &mut changed, &mut scroll_to_active);
                        ui.add_space(12.0);
                        self.render_pane_content(ui, id, &active_key, &mut add_contents);
                    } else {
                        self.render_pane_content(ui, id, &active_key, &mut add_contents);
        ...
        if let Some(user_key) = &mut self.active_key {
            if let Some(validated_key) = &active_key {
                if *user_key != validated_key {
...
impl<'a> Tabs<'a> {
    ...
    fn render_pane_content(&mut self, ui: &mut Ui, id: Id, active_key: &Option<String>, add_contents: &mut impl FnMut(&mut Ui, &str)) {
        if let Some(key) = active_key {
            ...
            let fade_value = ui.ctx().animate_bool_with_time(fade_id, true, ANIMATION_DURATION);
    ...
    fn get_tab_padding(&self) -> Vec2 {
        match (self.tab_type, self.size) {
            (TabType::Line, TabSize::Small) => {
                Vec2::new(12.0, 8.0)
            }
            (TabType::Line, TabSize::Medium) => {
                Vec2::new(16.0, 12.0)
            }
            (TabType::Line, TabSize::Large) => {
                Vec2::new(24.0, 16.0)
            }
    ...
    fn get_tab_styles(&self, ui: &Ui, pane: &TabPane, _parent_id: egui::Id, is_active: bool, is_hovered: bool) -> (Color32, egui::Margin, egui::CornerRadius) {
        ...
        let corner_radius = if is_card {
            match self.tab_position {
                TabPosition::Top => egui::CornerRadius { nw: 6, ne: 6, sw: 0, se: 0 },
                TabPosition::Bottom => egui::CornerRadius { nw: 0, ne: 0, sw: 6, se: 6 },
                TabPosition::Start => egui::CornerRadius { nw: 6, ne: 0, sw: 6, se: 0 },
                TabPosition::End => egui::CornerRadius { nw: 0, ne: 6, sw: 0, se: 6 },
            }
        } else {
...
impl<'a> Tabs<'a> {
    fn render_horizontal_tab_bar(&mut self, ui: &mut Ui, parent_id: Id, active_key: &mut Option<String>, changed: &mut bool, scroll_to_active: &mut bool) -> Option<TabEditAction> {
        let tabs_id = ui.id().with("horiz_tabs");
        let overflow_id = tabs_id.with("overflow");
        let scroll_command_id = tabs_id.with("scroll_command");

        let is_overflowing = ui.ctx().data(|d| d.get_temp::<bool>(overflow_id).unwrap_or(false));
        let mut scroll_command = ui.ctx().data(|d| d.get_temp::<Option<f32>>(scroll_command_id).flatten());
        ...
        ui.horizontal(|ui| {
            ...
            if is_overflowing {
                ...
                let can_scroll_left = if let Some(state) = egui::scroll_area::State::load(ui.ctx(), id) {
                    state.offset.x > 1.0
                } else {
                    false
                };
                ...
                if can_scroll_left {
                    if ui.add_sized([ARROW_WIDTH, ui.available_height()], egui::Button::new("<").frame(false)).on_hover_cursor(egui::CursorIcon::PointingHand).clicked() {
                        let id = tabs_id.with("scroll");
            ...
            let scroll_res = ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                ...
                if is_overflowing {
                    let current_max = ui.ctx().data(|d| d.get_temp::<f32>(tabs_id.with("current_max")).unwrap_or(0.0));
                    let current_offset = ui.ctx().data(|d| d.get_temp::<f32>(tabs_id.with("current_offset")).unwrap_or(0.0));
                    ...
                    scroll_area.max_width(ui.available_width()).show(ui, |ui| {
                        ...
                        if self.centered {
                            ui.vertical_centered(|ui| {
                                ui.horizontal(|ui| {
                                    self.render_tabs(ui, parent_id, active_key, changed, scroll_to_active)
                                }).inner
                            }).inner
                        } else {
                            self.render_tabs(ui, parent_id, active_key, changed, scroll_to_active)
        ...
        ui.ctx().layer_painter(LayerId::new(Order::Background, ui.id())).line_segment(
    ...
    fn render_vertical_tab_bar(&mut self, ui: &mut Ui, parent_id: Id, active_key: &mut Option<String>, changed: &mut bool, scroll_to_active: &mut bool) -> Option<TabEditAction> {
        ...
        let full_height = ui.available_height();
            let res = ui.vertical(|ui| {
                ...
                if is_overflowing {
                    let id = tabs_id.with("scroll");
                    let can_scroll_up = if let Some(state) = egui::scroll_area::State::load(ui.ctx(), id) {
                        state.offset.y > 1.0
                    } else {
                        false
                    };
                    ...
                    if can_scroll_up {
                        if ui.add_sized([ui.available_width(), ARROW_WIDTH], egui::Button::new("^").frame(false)).on_hover_cursor(egui::CursorIcon::PointingHand).clicked() {
                            let id = tabs_id.with("scroll");
                            let current_offset = egui::scroll_area::State::load(ui.ctx(), id).map(|s| s.offset.y).unwrap_or(0.0);
                            scroll_command = Some(current_offset - SCROLL_INCREMENT);
                            ui.ctx().data_mut(|d| d.insert_temp(scroll_command_id, scroll_command));
                        }
                    } else {
                ...
                let right_content = self.extra.right.take();
                let scroll_res = ui.with_layout(Layout::bottom_up(Align::Center), |ui| {
                    ...
                    if is_overflowing {
                        let current_max = ui.ctx().data(|d| d.get_temp::<f32>(tabs_id.with("current_max_v")).unwrap_or(0.0));
                        let current_offset = ui.ctx().data(|d| d.get_temp::<f32>(tabs_id.with("current_offset_v")).unwrap_or(0.0));
                        let can_scroll_down = current_offset < current_max - 1.0;

                        if can_scroll_down {
                            if ui.add_sized([ui.available_width(), ARROW_WIDTH], egui::Button::new("v").frame(false)).on_hover_cursor(egui::CursorIcon::PointingHand).clicked() {
                                let cmd = Some(current_offset + SCROLL_INCREMENT);
                                ui.ctx().data_mut(|d| d.insert_temp(scroll_command_id, cmd));
                            }
                        } else {
                    ...
                    ui.with_layout(Layout::top_down(Align::Center), |ui| {
                        ...
                        scroll_area.max_height(ui.available_height()).show(ui, |ui| {
        ...
        // Paint background line on a separate layer to avoid overlapping with active tab content
        ui.ctx().layer_painter(LayerId::new(Order::Background, ui.id())).line_segment(
            [egui::pos2(x, container_rect.min.y), egui::pos2(x, container_rect.max.y)],
            stroke
        );
    ...
    fn render_tabs(&mut self, ui: &mut Ui, parent_id: Id, active_key: &mut Option<String>, changed: &mut bool, scroll_to_active: &mut bool) -> Option<TabEditAction> {
        let has_focus = ui.memory(|m| m.has_focus(parent_id));
        self.render_tab_items(ui, parent_id, active_key, changed, scroll_to_active, has_focus)
    }

    fn render_tab_items(&mut self, ui: &mut Ui, parent_id: Id, active_key: &mut Option<String>, changed: &mut bool, scroll_to_active: &mut bool, has_focus: bool) -> Option<TabEditAction> {
        let mut edit_action = None;
        ...
        for (i, pane) in self.panes.iter().enumerate() {
            ...
            let last_rect = ui.data(|d| d.get_temp::<egui::Rect>(tab_id).unwrap_or(egui::Rect::NOTHING));
            ...
            let (text_color, margin, corner_radius) = self.get_tab_styles(ui, pane, parent_id, is_active, is_hovered);

            ui.push_id(&pane.key, |ui| {
                let mut close_clicked = false;
                let (tab_rect, label_rect) = ui.add_enabled_ui(!pane.disabled, |ui| {
                    let is_card = self.tab_type == TabType::Card || self.tab_type == TabType::EditableCard;
                            ...
                            ui.horizontal(|ui| {
                                ...
                                ui.scope(|ui| {
                                    ...
                                    let font_id = if is_active {
                                        let mut id = ui.style().text_styles.get(&egui::TextStyle::Heading)
                                            .cloned()
                                            .unwrap_or_else(|| egui::FontId::proportional(self.get_font_size()));
                                ...
                                if (self.tab_type == TabType::EditableCard) && pane.closable {
                                    ...
                                    let color = if is_close_hovered { ANTD_BLUE } else { color };

                                    let close_btn = egui::Button::new(
                                        egui::RichText::new(egui_phosphor::regular::X).size(12.0).color(color)
                                    ).frame(false);

                                    let close_resp = ui.add(close_btn).on_hover_cursor(egui::CursorIcon::PointingHand);

                                    // Save hover state for the next frame
                                    ui.data_mut(|d| d.insert_temp(close_id, close_resp.hovered()));
                ...
                if is_active {
                    if has_focus {
                        ...
                        ui.painter().rect_stroke(focus_rect, corner_radius, egui::Stroke::new(2.0, ANTD_BLUE.gamma_multiply(0.3)), egui::StrokeKind::Outside);
                    }

                    if self.tab_type == TabType::Line {
                        let ink_id = parent_id.with("ink_bar");

                        let (target_min, target_max) = if self.tab_position == TabPosition::Start || self.tab_position == TabPosition::End {
                            (label_rect.min.y, label_rect.max.y)
                        } else {
                            (label_rect.min.x, label_rect.max.x)
                        };

                        let anim_min = ui.ctx().animate_value_with_time(ink_id.with("min"), target_min, ANIMATION_DURATION);
                        let anim_max = ui.ctx().animate_value_with_time(ink_id.with("max"), target_max, ANIMATION_DURATION);

                        let line_rect = if self.tab_position == TabPosition::Start || self.tab_position == TabPosition::End {
                            let x = if self.tab_position == TabPosition::Start { tab_rect.max.x } else { tab_rect.min.x };
                            let x_offset = if self.tab_position == TabPosition::Start { -2.0 } else { 2.0 };
                            egui::Rect::from_min_max(
                                egui::pos2(x + x_offset - 1.0, anim_min),
                                egui::pos2(x + x_offset + 1.0, anim_max)
                            )
                        } else {
                            let y = if self.tab_position == TabPosition::Top { tab_rect.max.y } else { tab_rect.min.y };
                            let y_offset = if self.tab_position == TabPosition::Top { -2.0 } else { 2.0 };
                            egui::Rect::from_min_max(
